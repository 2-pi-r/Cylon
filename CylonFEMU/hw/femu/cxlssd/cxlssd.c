...
static uint16_t get_lsa(struct FemuCtrl *n, void *buf, uint64_t size, uint64_t offset)
{
    ...
    case 1://print hit/miss count
        ...
        fprintf(f,"Buffer read: %lu hit_trapped/ %lu miss\n", buffer->load_hit_trapped, buffer->load_miss);
        fprintf(f,"Buffer write: %lu hit_trapped/ %lu miss\n", buffer->store_hit_trapped, buffer->store_miss);
        buffer->ins_cnt = buffer->evict_cnt = buffer->load_hit_trapped = buffer->load_miss = buffer->store_hit_trapped = buffer->store_miss = 0;
