...
static int ct3_build_cdat_table(CDATSubHeader ***cdat_table, void *priv)
{
    ...
    if (ct3d->femu) {
        ...
        if (!volatile_mr) {  
#elif
        nonvolatile_mr = &ct3d->femu->cxl_mr;
        if (!nonvolatile_mr) { 
#endif
            return -EINVAL;
        }
...
static void build_dvsecs(CXLType3Dev *ct3d)
{
    ...
    uint8_t *dvsec;
    /* 
     * CXL specification 8.1.3.8.2 (page 380)
     *
     * 
     * R1-Range 1 Memory Size (64bit)
     * [.....range1_size_hi......|.....|..........................................]
     * 63                      32|31 28|                                          0
     *                              |
     *                              V   27    16 15  13 12       8 7   5 4  2 1 0     
     *       range1_size_lo -   [|.....|reserved|HwInit|Interleave|Class|Type|.|.|]
     * 
     * 
     * 
     * 
    ...
    if (ct3d->hostvmem) {
        ...
        // uint64_t size = (uint64_t)ct3d->femu->bufsz*MiB;
...
static void ct3_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    printf("CXL Type3 Device Realizing\n");
                           ...
                           unsigned size, MemTxAttrs attrs)
{
    ...
    if (ct3d->femu){
        ...
        // print_now("ct3 dpa");
        return ct3d->femu->cxl_mem_ops.read(ct3d->femu, dpa_offset, data, size, attrs);    
    }
                            ...
                            unsigned size, MemTxAttrs attrs)
{
    ...
    if (ct3d->femu) {
        ...
        return ct3d->femu->cxl_mem_ops.write(ct3d->femu, dpa_offset, data, size, attrs);
    }
...
static Property ct3_props[] = {
    ...
    DEFINE_PROP_LINK("lsa", CXLType3Dev, lsa, TYPE_MEMORY_BACKEND,
                     HostMemoryBackend *),
    DEFINE_PROP_LINK("femu", CXLType3Dev, femu, TYPE_FEMU,
                     FemuCtrl *),
...
static MemoryRegion* ct3d_get_lsa_mr(CXLType3Dev *ct3d)
{
...
{
    MemoryRegion *mr = ct3d_get_lsa_mr(ct3d);
...
static bool set_cacheline(CXLType3Dev *ct3d, uint64_t dpa_offset, uint8_t *data)
{
    MemoryRegion *vmr = NULL, *pmr = NULL;
    AddressSpace *as;
    printf("set_cacheline: dpa_offset: 0x%lx\n", dpa_offset);

    if (ct3d->femu) { 
        return ct3d->femu->cxl_mem_ops.write(ct3d->femu, dpa_offset, (uint64_t)data, CXL_CACHE_LINE_SIZE, MEMTXATTRS_UNSPECIFIED);
    }
...
#include "hw/femu/log_time.h"
static uint64_t now__;
void update_time(void)
{
    now__ = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
}

void print_now(const char *name)
{
    uint64_t time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME)-now__; 
    printf("%s: %ld\n",name, time);
    update_time();
}